...
// Cloudflare Workers and the Edge runtime always give `request.url` as an
// absolute URL; Vercel's Node.js runtime gives only the path, so `new
// URL(request.url)` throws there. This works on both.
export function requestUrl(request) {
  if (/^https?:\/\//u.test(request.url)) return new URL(request.url);
