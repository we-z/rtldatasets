...
import { assertMethod, assertSameOrigin, clientIp, HttpError, readUrlEncodedForm, redirect } from './http.js';
...
export async function createCheckout(request, env) {
  ...
  const rate = await checkRateLimit(env, 'checkout', rateKey);
  if (!rate.success) throw new HttpError(429, 'checkout_rate_limited');
  await loadVerifiedArtifact(config, env);
