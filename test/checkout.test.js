...
import { createCheckout } from '../worker/checkout.js';
import { publicErrorResponse } from '../worker/handlers.js';

test('disabled checkout fails closed before requiring production secrets', async () => {
  const request = new Request('https://www.rtldatasets.com/api/create-checkout-session', {
    method: 'POST',
  });
...
test('configuration failures produce a safe unavailable response', () => {
  ...
  const response = publicErrorResponse(
    new Request('https://www.rtldatasets.com/api/create-checkout-session'),
  ...
  assert.equal(
    ...
    'https://www.rtldatasets.com/purchase-error?reason=store_not_configured',
  );
});
