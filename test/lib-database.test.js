...
import { findRecoverablePurchase, recordDownload, recordPurchase } from '../lib/database.js';
import { PRODUCT } from '../lib/product.js';

function fakePool(queryImpl) {
  return { __dbPoolOverride: { query: queryImpl } };
}

test('purchase records use the protected asset path and remain idempotent', async () => {
  const calls = [];
  const env = fakePool(async (sql, values) => {
    calls.push({ sql, values });
    if (sql.trim().startsWith('SELECT')) {
      return { rows: [{ checkout_session_id: 'cs_test_example' }] };
    }
    return { rowCount: 1 };
  });
  const paid = {
...
test('delivery timestamp is written synchronously and fails closed without one row', async () => {
  const calls = [];
  const environment = (rowCount) => fakePool(async (sql, values) => {
    calls.push({ sql, values });
    return { rowCount };
  });
  ...
  assert.match(calls[0].sql, /first_download_at = COALESCE\(first_download_at, \$1\)/u);
...
test('recovery lookup is bounded by product, mode, trusted time, and remembered attempts', async () => {
  ...
  assert.match(calls[0].sql, /stripe_created_at >= \$3/u);
